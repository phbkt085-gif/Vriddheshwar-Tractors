...
import { Loader2, Upload, AlertCircle, Trash2, LogOut, Folder, Image as ImageIcon, Plus, Edit2, X, Box } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const [modelForm, setModelForm] = useState({ id: 0, name: '', hp: '', feature: '', marathi: '', img: '' });
  ...
  const fetchModels = async () => {
    if (!supabase) return;
    const { data } = await supabase.storage.from('public-images').download('models_data.json');
    if (data) {
      try {
  ...
  const saveModels = async (newModels: any[]) => {
    if (!supabase) return;
    const file = new Blob([JSON.stringify(newModels)], { type: 'application/json' });
    const { error } = await supabase.storage.from('public-images').upload('models_data.json', file, { upsert: true });
    if (!error) {
  ...
  const handleModelSubmit = (e: React.FormEvent) => {
    ...
    let newModels;
    if (isEditingModel) {
      newModels = modelsData.map(m => m.id === modelForm.id ? modelForm : m);
    } else {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!supabase) {
      setAuthError('Supabase is not configured yet. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY.');
      return;
    }
    setLoading(true);
    setAuthError('');
  ...
  const fetchImages = async (section: string) => {
    if (!supabase) return;
    const { data, error } = await supabase.storage.from('public-images').list(section);
    if (data) {
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    const { error } = await supabase.storage.from('public-images').remove([`${activeSection}/${imageToDelete}`]);
    if (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 text-center">
        <div className="max-w-md bg-white p-8 rounded-xl shadow border border-red-200">
          ...
          <p className="text-slate-600 mb-6 text-sm">Please configure VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in your env secrets to enable admin features.</p>
          <button onClick={() => navigate('/')} className="px-4 py-2 bg-slate-900 text-white rounded-lg">Return Home</button>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
        <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-xl">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-slate-500 mt-2">Only authorized personnel</p>
          </div>
          ...
          <div className="mt-6 text-center">
             <button onClick={() => navigate('/')} className="text-sm text-slate-500 hover:text-slate-800">Return to Website</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <header className="bg-white border-b sticky top-0 z-20 px-6 py-4 flex justify-between items-center shadow-sm">
        ...
        <div className="flex gap-4">
          <button onClick={() => navigate('/')} className="px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 rounded-lg font-medium">View Site</button>
          <button 
        ...
        <aside className="w-64 bg-white border-r h-full flex flex-col hidden md:flex">
          ...
          <nav className="flex-1 p-4 space-y-2">
            {SECTIONS.map(s => (
              ...
              >
                <Folder className={`w-4 h-4 ${activeSection === s.id ? 'text-red-500' : 'text-slate-400'}`} />
                {s.label}
              </button>
            ))}
          </nav>
          ...
          <div className="max-w-5xl mx-auto">
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button 
                    onClick={() => {
                      ...
                      setModelForm({ id: 0, name: '', hp: '', feature: '', marathi: '', img: '' });
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {modelsData.map(m => (
                        <tr key={m.id} className="hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3 text-slate-600 truncate max-w-[200px]">{m.feature} / {m.marathi}</td>
                          <td className="px-4 py-3 text-slate-600 truncate max-w-[150px]">{m.img}</td>
              ...
              <div className="flex items-center justify-center w-full">
                <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-slate-300 border-dashed rounded-xl cursor-pointer bg-slate-50 hover:bg-slate-100 transition-colors">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-xs text-slate-500 mt-1">Uploading to <strong>{activeSection}</strong> folder</p>
                  </div>
            ...
            {files.length === 0 ? (
              <div className="text-center p-12 bg-white rounded-2xl border border-slate-200 border-dashed flex flex-col items-center justify-center">
                ...
                <p className="text-slate-600 font-medium">No images uploaded to this section yet.</p>
                <p className="text-slate-500 text-sm mt-1">Upload photos above to securely display them on the website.</p>
              </div>
            ) : (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                {files.map((file) => (
                  <div key={file.name} className="group relative bg-white rounded-xl shadow-sm overflow-hidden aspect-video md:aspect-square border border-slate-200">
                    <img 
                    ...
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <button onClick={() => setImageToDelete(file.name)} className="px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-red-700 shadow-lg transform hover:scale-105 transition-all">
                        <Trash2 className="w-4 h-4" /> Delete
                      </button>
                    </div>
                    <div className="absolute bottom-0 w-full bg-gradient-to-t from-black/80 to-transparent p-3 pt-6 pointer-events-none">
                       <p className="text-[10px] sm:text-xs text-white/90 truncate font-medium">{file.name}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {imageToDelete && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl">
            ...
            <p className="text-slate-500 mb-6">Are you sure you want to delete this image? This action cannot be undone.</p>
      ...
      {showModelModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-lg w-full shadow-xl">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-slate-900">{isEditingModel ? 'Edit Model' : 'Add Model'}</h3>
            ...
            <form onSubmit={handleModelSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" required value={modelForm.name} onChange={e => setModelForm({...modelForm, name: e.target.value})} className="w-full px-3 py-2 rounded-lg border focus:ring-2 focus:ring-red-200 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Horsepower (HP)</label>
                  <input type="text" required value={modelForm.hp} onChange={e => setModelForm({...modelForm, hp: e.target.value})} className="w-full px-3 py-2 rounded-lg border focus:ring-2 focus:ring-red-200 outline-none" />
                </div>
              </div>
              
              <div>
                <label className="block text-sm font-medium mb-1">Feature (English)</label>
                <input type="text" required value={modelForm.feature} onChange={e => setModelForm({...modelForm, feature: e.target.value})} className="w-full px-3 py-2 rounded-lg border focus:ring-2 focus:ring-red-200 outline-none" />
              </div>

              <div>
                <label className="block text-sm font-medium mb-1">Feature (Marathi)</label>
                <input type="text" required value={modelForm.marathi} onChange={e => setModelForm({...modelForm, marathi: e.target.value})} className="w-full px-3 py-2 rounded-lg border focus:ring-2 focus:ring-red-200 outline-none" />
              </div>

              <div>
                <label className="block text-sm font-medium mb-1">Image <span className="text-xs text-slate-500 font-normal">(Upload from device or paste URL)</span></label>
                <div className="flex gap-2 items-center mb-2">
                  <label className="cursor-pointer bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-lg text-sm font-medium text-center transition-colors shadow-sm border border-slate-200 shrink-0">
                    {uploadingModelImage ? (
                      ...
                      <span className="flex items-center justify-center gap-2"><Upload className="w-4 h-4" /> Upload File</span>
                    )}
                    <input type="file" accept="image/*" className="hidden" onChange={handleModelImageUpload} disabled={uploadingModelImage} />
                  </label>
                  <span className="text-slate-400 text-xs font-semibold shrink-0">OR</span>
                  <input type="text" value={modelForm.img} onChange={e => setModelForm({...modelForm, img: e.target.value})} placeholder="https://..." className="flex-1 px-3 py-2 rounded-lg border focus:ring-2 focus:ring-red-200 outline-none text-sm w-full" />
                </div>
                ...
                {modelForm.img && (
                  <div className="mt-2 text-sm text-slate-600 bg-slate-50 p-2 rounded-lg border border-slate-100 flex items-center justify-between">
                     <span className="truncate flex-1 mr-4 text-xs">{modelForm.img}</span>
                     {modelForm.img.startsWith('http') || modelForm.img.startsWith('/') ? (
                       <img src={modelForm.img} alt="Preview" className="w-10 h-10 object-cover rounded shadow-sm border border-slate-200 shrink-0" />
                     ) : null}
                  </div>
                )}
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4 mt-6 border-t border-slate-100">
                <button type="button" onClick={() => setShowModelModal(false)} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg font-medium">Cancel</button>
                <button type="submit" className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium">Save Model</button>
              </div>
            </form>
          </div>
        </div>
